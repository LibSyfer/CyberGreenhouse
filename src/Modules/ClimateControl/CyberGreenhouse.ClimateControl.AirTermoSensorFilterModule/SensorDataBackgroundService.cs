...
namespace CyberGreenhouse.ClimateControl.AirHumiditySensorFilterModule
{
    public class SensorDataBackgroundService : BackgroundService
    {
        ...
        private readonly AirTempreatureSensor _sensors;
        private readonly IMessageBus _messageBus;
        private readonly int _sensorsCount;

        public SensorDataBackgroundService(ILogger<SensorDataBackgroundService> logger, AirTempreatureSensor sensors, IMessageBus messageBus, int sensorsCount = 3)
        {
            _logger = logger;
            _sensors = sensors;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                await _messageBus.SendAsync(ModuleNames.ClimateControl, new AirTemperatureEvent
                {
